...
async function main() {
  ...
  const content = String(
    ti.new_string ?? ti.content ?? ti.file_text ?? ti.command ?? ""
  );
  const path = String(
...
function failOpen(datumDir, message) {
  try {
    ...
    appendFileSync(
      ...
      `${(/* @__PURE__ */ new Date()).toISOString()} fence fail-open: ${message}
`
    );
  } catch {
  }
...
async function postFenced(busUrl, payload) {
  const ac = new AbortController();
...
function matchedDelta(deltas, content) {
  for (const d of deltas) {
    const mc = d.mechanical_change;
    const stale = mc.kind === "rename_column" ? mc.from : mc.kind === "drop_column" ? mc.column : mc.kind === "api_field_renamed" || mc.kind === "api_field_removed" ? mc.from : void 0;
    if (stale && wordBoundary(content, stale)) return d;
  }
  return deltas[0];
}
function matchedEpoch(deltas, content) {
  return matchedDelta(deltas, content)?.epoch;
}
function matchedContractId(deltas, content) {
  return matchedDelta(deltas, content)?.contract_id;
}
function wordBoundary(content, symbol) {
  const escaped = symbol.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
...
var isMain = runAsEntry(import.meta.url, "fence");
if (isMain) {
  main().catch(() => {
