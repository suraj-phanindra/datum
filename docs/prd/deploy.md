...
- **`web/snapshot.json` is regenerated only AFTER arbiter + spec-pr event payloads are frozen** (RECONCILIATION gate 3). The acceptance check asserts `snapshot.registry_version === 8` and that the replay contains all six event types through `spec.pr.opened` #14.
