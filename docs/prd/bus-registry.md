...
### Visible behavior
This layer is the source of truth others render. With the web app stopped, a terminal call to `POST /events` carrying asha's edit to `migrations/0042_*.sql` (`users.email` -> `contact_email`) returns `{ ok, registry_version: 8, delta }`; `GET /version` now reports `{ registry_version: 8 }`. A second `POST /events` for a `README.md` or `*.test.ts` edit returns the same epoch unchanged. `GET /registry` shows `db.users` at `current_version` 8 alongside `api.GET /users/:id` v3, `api.POST /invites` v1, `deps.db-driver` v2. `GET /stream` emits a live `delta.detected` Event. The epoch ticks 7 -> 8 exactly once.
