...
### Acceptance test
`test/stop-guard.test.ts`. Satisfies RUBRIC "Nice to have": *"Stop hook prevents an agent from declaring done while unacknowledged deltas intersect its diff."* Assertions: (1) session behind by one epoch with diff containing `.email` and the seeded `db.users` v7→v8 delta → guard returns block, reason names `db.users`, `email→contact_email`, `asha`; (2) session at `last_synced_version === registry_version` → allow, no `/deltas` call (cache-hit fast path); (3) diff already using `contact_email` → allow; (4) bus down → allow (fail-open) + warning logged.
