...
### Acceptance test
`test/tower.test.ts` satisfies RUBRIC "The live URL returns 200 and shows the registry at v8." With a seeded bus running, start `web/serve.ts`, `GET /` asserts HTTP 200, and the served HTML/hydration asserts the rendered epoch reads `v8` and the `db.users` row reads `v8`. A second case satisfies the not-a-dashboard check: with `web/serve.ts` stopped, the demo-runner fence/advisory/spec-PR assertions still pass.
