...
- **The advisory/PR stub is flag-isolated test scaffold only** — the real arbiter + real spec-pr are exercised by their own acceptance runs (RECONCILIATION gate 2). `reconciled` in the live run comes from the server (bus-registry), not the emitter.
