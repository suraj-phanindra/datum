...
### Acceptance test
`test/spec-pr.test.ts`. Satisfies the acceptance requirement that the arbiter opens a real PR that patches docs/spec.md to the new contract, with a linked ledger entry (and the `datum demo` end-to-end check that one spec PR is opened). Assertions: given the hero delta, `openSpecPR` produces a patch that removes `email` and adds `contact_email` in `docs/spec.md`; the linked `LedgerEntry` (#112) exists with `contract_id === "db.users"`; exactly one `spec.pr.opened` event is emitted with `patch_path === "docs/spec.md"`, `epoch === 8`, and a non-empty `pr_number`/`url`. Runs with the web app stopped (not-a-dashboard).
